...
```{r OAS1}
    ...
    res0         <- multiseq(x=OAS1$x[OAS1$g==0,], read.depth=OAS1$read.depth[OAS1$g==0])
    res1         <- multiseq(x=OAS1$x[OAS1$g==1,], read.depth=OAS1$read.depth[OAS1$g==1])
    res2         <- multiseq(x=OAS1$x[OAS1$g==2,], read.depth=OAS1$read.depth[OAS1$g==2])
    res          <- multiseq(x=OAS1$x, g=OAS1$g, read.depth=OAS1$read.depth)
...
```{r testing_on_chipseq}
      ...
      res0        <- multiseq(x=x[which(g==0),], read.depth=samples$ReadDepth[which(g==0)])
      res1  	  <- multiseq(x=x[which(g==1),], read.depth=samples$ReadDepth[which(g==1)]) 
      #find an effect given a covariate
      res         <- multiseq(x=x, g=g, read.depth=samples$ReadDepth)
