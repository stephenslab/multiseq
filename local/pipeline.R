...
for (i in 1:nregions){
    ...
    #res    <- multiseq(M, g=g, minobs=1, lm.approx=FALSE, read.depth=samples$ReadDepth, smoothing=FALSE, cyclespin=FALSE, get.fitted.g=FALSE)
...
for (j in 1:J)
    y=cbind(y,y.list[[j]])
res    <- multiseq(listy=list(y=y, y.rate=y.rate, intervals=intervals), g=g, cyclespin=FALSE, get.fitted.g=TRUE) #specify 

#qsub to each locus
multiseq(M,fitted.g=res$scales$fitted.g, fitted.g.intercept=res$scales$fitted.g.intercept)
